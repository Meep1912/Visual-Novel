...
# --- Scale helpers ---
BW, BH = 1280, 720
def sx(v): return int(v * screen.get_width() / BW)
def sy(v): return int(v * screen.get_height() / BH)
# --- Font ---
...
def draw_game(text_line1, text_line2):
    ...
    fastforward = Buttonify("Assets/undo-button1.png", (sx(900), sy(550)), False)
...
def draw_characters(char1, char2):
    ...
    if char1 == "Empty.png" and char2 == "Empty.png":
        ...
        screen.blit(char1, (x + sx(768), y))
    elif char1 != "Empty.png" and char2 != "Empty.png":
        char1 = pygame.image.load(char1)
        screen.blit(char1, (x, y))
        char2 = pygame.image.load(char2)
        screen.blit(char2, (x + sx(764), y)) # originaly 864

def fade(NextBG, currentBG):
    old = pygame.image.load(currentBG).convert_alpha()
    old = pygame.transform.scale(old, screen.get_size())
    new = pygame.image.load(NextBG).convert_alpha()
    new = pygame.transform.scale(new, screen.get_size())
